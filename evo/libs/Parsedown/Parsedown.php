<?php
###### FEATURE ADDED BY EVO, CAREFUL IF UPDATED ######
/*
	Image alignment and friendlier title: 
	![title](url right|left|center)
	![alt](url "title" right|left|center)
	
	Wiki style headers with = 
	= H1 =
	== H2 == 
*/
...
class Parsedown
{
    ...
    protected function identifyAtx($Line)
    {
        if (isset($Line['text'][1]))
        {
            $level = 1;
				...
				$char = $Line['text'][0] === '=' ? '=' : '#';
            while (isset($Line['text'][$level]) and $Line['text'][$level] === $char)
            {
                $level ++;
            }

            $text = trim($Line['text'], $char.' ');
    ...
    protected function identifyLink($Excerpt)
    {
        ...
        if (strpos($Excerpt['text'], ']') and preg_match('/\[((?:[^][]|(?R))*)\]/', $Excerpt['text'], $matches))
        {
            ...
            elseif (preg_match('/^\([ ]*(.*?)(?:[ ]+[\'"](.+?)[\'"])?[ ]*(left|right|center)?[ ]*\)/', $substring, $matches))
            {
                $Link['url'] = $matches[1];
        ...
        else
        {
			...
			$Element['attributes']['title'] = empty($Link['title']) ? @$Element['attributes']['alt'] : $Link['title'];
		  
        if (!empty($Link['align']))
        {
            $Element['attributes']['align'] = $Link['align'];
        }

...
$steps = array('Bienvenue !', 'Acceptation', 'Informations SQL', 'Configuration', 'Terminé');
$next_step = $cur_step;
...
switch($cur_step) {
	case 0:
		...
		if (!in_array(false, $ok, false)) $next_step = 1; else $next_step = 1000;
		break;
	case 1:
		$next_step = 2;
		break;
	...
	case 3:
		if (isset($_POST['email'], $_POST['admin'], $_POST['admin_pass'], $_POST['url'], $_POST['name'], $_POST['payload'])) {
			...
			try {
				...
				Db::Exec('insert into {settings} (name, value) values ("name", ?), ("email", ?), ("url", ?), ("theme", "default"), ("database.version", ?), ("salt", ?)',
							$_POST['name'], $_POST['email'], $_POST['url'], DATABASE_VERSION, uniqid(sha1(rand()), true));
